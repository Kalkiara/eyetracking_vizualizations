...
Fixations <- read_csv("C:/Users/maria/OneDrive/Dokumenter/Uni/Models for Perception and Action/Eye-Tracking-Workhop-CogSci-F2021-main/data/Fixations_final_flipped.csv", guess_max = Inf)
Saccades <- read_csv("C:/Users/maria/OneDrive/Dokumenter/Uni/Models for Perception and Action/Eye-Tracking-Workhop-CogSci-F2021-main/data/Saccades_final_flipped.csv")
...
make_scanpath <- function(participant, trial){
  ...
  img <- jpeg::readJPEG(paste0('C:/Users/maria/OneDrive/Dokumenter/Uni/Models for Perception and Action/Eye-Tracking-Workhop-CogSci-F2021-main/data/stimuli_Foraging/', x$Stimulus[1])) 
...
```{r}
for ( j in unique(Fixations$Trial)) {
  plot <- make_scanpath("mia2", j)
}
...
```{r}
# for ( j in unique(Fixations$ParticipantID)) {
#   plot <- make_heatmap(j,3)
# }

Fixations2 <- Fixations %>% 
  filter(Task == "Visual Foraging")

intheheatofthenight <- function(trial) {

x <- Fixations2[Fixations2$Trial == trial,]
  ...
  p <- ggplot(x, aes(PositionX, PositionY, color = FixationNo)) + 
    annotation_custom(img, xmin = 0, xmax = 1680, ymin = 0, ymax = 1050) +
    #making a white square to lay on top which makes the pictur opaque
    annotate(geom = "rect", xmin = 0, xmax = 1680, ymin = 0, ymax = 1050, fill = "white", alpha = .3) +
    # the next line is the density function (needed to plot the different colors)
    #2d density instead of 1d
    stat_density2d(geom = "raster", aes(fill = ..density.., alpha = sqrt(sqrt(..density..))), contour = FALSE, n = 1000) +
    # we take the density data and apply the colors that we chose
    scale_fill_gradientn(colours = heat_colors(10), trans="sqrt") +
    # and apply dynamic alpha to it so that the fewer the fixations, the more transparent the color
...
intheheatofthenight(8) #penguins

intheheatofthenight(10) #space capsules
...
img <- jpeg::readJPEG('C:/Users/maria/OneDrive/Dokumenter/Uni/Models for Perception and Action/Eye-Tracking-Workhop-CogSci-F2021-main/data/stimuli_Foraging/sheep.jpg')  
img <- grid::rasterGrob(img, width=unit(1, "npc"), height = unit(1,"npc"), interpolate = FALSE)
...
intheheatofthenight2 <- function(trial, AOI) {
...
aoi_dolphin <- c(100, 500, 100, 500)

intheheatofthenight2(7, aoi_dolphin) #dolphins
...
img <- jpeg::readJPEG('C:/Users/maria/OneDrive/Dokumenter/Uni/Models for Perception and Action/Eye-Tracking-Workhop-CogSci-F2021-main/data/stimuli_Foraging/trees.jpg')  
img <- grid::rasterGrob(img, width=unit(1, "npc"), height = unit(1,"npc"), interpolate = FALSE)
